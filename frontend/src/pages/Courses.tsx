...
export function CoursesPage() {
  ...
  useEffect(() => { const t = setTimeout(() => api.get<{ catalogs: CatalogSummary[] }>(`/courses?q=${encodeURIComponent(q)}`).then((r) => setCatalogs(r.catalogs)), 200); return () => clearTimeout(t); }, [q]);
...
export function CourseDetailPage() {
  ...
  useEffect(() => { api.get<{ catalog: Catalog }>(`/courses/${id}`).then((r) => setCatalog(r.catalog)); }, [id]);
